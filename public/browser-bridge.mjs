...
const LOCAL_ROOT = path.resolve(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'));
const SESSION_ROOT = path.join(LOCAL_ROOT, 'FirecrawlLocal', 'browser-profiles');
...
function resolveChrome() {
  ...
  for (const candidate of candidates) {
    ...
    if (path.basename(resolved).toLowerCase() !== 'chrome.exe') continue;
...
function sanitizeProductPayload(value, depth = 0) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (++count > 300) break;
    if (SENSITIVE_RESPONSE_KEY.test(key)) { out[key] = '[REDACTED]'; continue; }
...
async function cdpHealth(cfg) {
  try {
    ...
    if (ws.protocol !== 'ws:' || ws.hostname !== '127.0.0.1' || Number(ws.port) !== cfg.port || !ws.pathname.startsWith('/devtools/browser/')) throw new Error('unexpected debugger URL');
    return { healthy: true, browser: String(data.Browser || ''), protocolVersion: String(data['Protocol-Version'] || '') };
  } catch (error) { return { healthy: false, error: 'CDP unavailable' }; }
}
...
class CdpClient {
  constructor(wsUrl, sessionName, cfg) {
    ...
    if (u.protocol !== 'ws:' || u.hostname !== '127.0.0.1' || Number(u.port) !== cfg.port || !/^\/devtools\/page\/[A-Za-z0-9_-]+$/.test(u.pathname)) throw new Error('invalid page debugger URL');
    this.sessionName = sessionName; this.cfg = cfg; this.wsUrl = u.href; this.ws = null; this.nextId = 1; this.pending = new Map(); this.network = []; this.maxNetwork = 1000;
  }
  async open() {
    ...
    await this.send('Runtime.enable'); await this.send('Page.enable'); await this.send('Network.enable', { maxTotalBufferSize: 5_000_000, maxResourceBufferSize: 1_000_000 });
  }
  onMessage(event) {
    ...
    if (Number.isInteger(msg.id)) {
      ...
      if (msg.error) p.reject(new Error(String(msg.error.message || 'CDP command failed'))); else p.resolve(msg.result || {}); return;
    }
    const p = msg.params || {};
    if (msg.method === 'Network.requestWillBeSent') {
      const r = p.request || {}; this.pushNetwork({ ts: Date.now(), kind: 'request', requestId: String(p.requestId || ''), method: String(r.method || ''), url: redactUrl(r.url), resourceType: String(p.type || '') });
    } else if (msg.method === 'Network.responseReceived') {
      const r = p.response || {}; this.pushNetwork({ ts: Date.now(), kind: 'response', requestId: String(p.requestId || ''), url: redactUrl(r.url), status: Number(r.status || 0), mimeType: String(r.mimeType || ''), resourceType: String(p.type || '') });
    }
  }
  pushNetwork(item) { if (!allowedUrl(item?.url, this.cfg)) return; this.network.push(item); if (this.network.length > this.maxNetwork) this.network.splice(0, this.network.length - this.maxNetwork); }
  send(method, params = {}) {
  ...
  async evaluate(expression) { const result = await this.send('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true }); if (result.exceptionDetails) throw new Error('evaluation failed'); return result.result?.value; }
  close() { try { this.ws?.close(); } catch {} this.failAll(new Error('CDP detached')); }
}
async function attach(sessionName) {
  const cfg = sessionConfig(sessionName); const targets = await listTargets(cfg);
  if (targets.length < 1) throw new Error('no matching page target');
  const target = targets[0]; const prior = states.get(sessionName);
  if (prior?.targetId === target.id && prior.client?.ws?.readyState === WebSocket.OPEN) return prior;
  prior?.client?.close(); const client = new CdpClient(String(target.webSocketDebuggerUrl || ''), sessionName, cfg); await client.open();
  const state = { client, targetId: String(target.id || ''), attachedAt: new Date().toISOString() }; states.set(sessionName, state); return state;
}
async function ensureSession(sessionName) {
  const cfg = sessionConfig(sessionName); let health = await cdpHealth(cfg); let started = false;
  if (!health.healthy) {
    const profileDir = path.resolve(SESSION_ROOT, sessionName);
    const rootPrefix = SESSION_ROOT.endsWith(path.sep) ? SESSION_ROOT : SESSION_ROOT + path.sep;
    if (!profileDir.startsWith(rootPrefix)) throw new Error('session profile path escaped browser profile root');
    fs.mkdirSync(profileDir, { recursive: true });
    const args = [ '--remote-debugging-address=127.0.0.1', `--remote-debugging-port=${cfg.port}`, `--user-data-dir=${profileDir}`, '--profile-directory=Default', '--new-window', '--no-first-run', '--no-default-browser-check', '--start-maximized', cfg.startUrl ];
    const child = spawn(resolveChrome(), args, { detached: true, stdio: 'ignore', windowsHide: false }); child.unref(); started = true;
    const deadline = Date.now() + 15_000;
    while (Date.now() < deadline) { await new Promise(r => setTimeout(r, 250)); health = await cdpHealth(cfg); if (health.healthy) break; }
    if (!health.healthy) throw new Error('Chrome CDP did not become ready');
  }
  let state = null; const deadline = Date.now() + 10_000;
  while (Date.now() < deadline) { try { state = await attach(sessionName); break; } catch { await new Promise(r => setTimeout(r, 250)); } }
...
async function status(sessionName) {
  const cfg = sessionConfig(sessionName); const health = await cdpHealth(cfg); let targets = [];
  if (health.healthy) { try { targets = (await listTargets(cfg)).map(t => ({ id: String(t.id || ''), title: String(t.title || ''), url: redactUrl(t.url) })); } catch {} }
...
function directProductUrl(sessionName, productId) {
  const id = String(productId || ''); if (!/^[0-9]{1,12}$/.test(id)) throw new Error('invalid product ID');
...
function directCategoryUrl(sessionName, pageNumber) {
  ...
  const page = Number(pageNumber); if (!Number.isInteger(page) || page < 1 || page > 100) throw new Error('invalid category page number');
  return `https://ehandel.spendrups.se/c/drycker/sprit/all-sprit?page=${page}`;
}
async function openCategory(sessionName, pageNumber) {
  const cfg = sessionConfig(sessionName); const { client } = await attach(sessionName); const url = directCategoryUrl(sessionName, pageNumber); const parsed = new URL(url);
  ...
  const deadline = Date.now() + 15_000; let value = null;
  while (Date.now() < deadline) { await new Promise(r => setTimeout(r, 250)); try { value = await client.evaluate('({ url: location.href, title: document.title, readyState: document.readyState })'); if (value?.readyState === 'complete') break; } catch {} }
...
async function openProduct(sessionName, productId) {
  const cfg = sessionConfig(sessionName); const { client } = await attach(sessionName); const url = directProductUrl(sessionName, productId); const parsed = new URL(url);
...
async function snapshot(sessionName) {
  ...
  const value = await client.evaluate(`(() => {
    ...
    const inputs = Array.from(document.querySelectorAll('input,select,textarea')).slice(0,100).map((e,i)=>({i, tag:e.tagName.toLowerCase(), type:e.getAttribute('type')||null, name:e.getAttribute('name')||null, id:e.id||null, placeholder:e.getAttribute('placeholder')||null, aria:e.getAttribute('aria-label')||null}));
  ...
  const cfg = sessionConfig(sessionName); value.url = redactUrl(value.url);
...
async function productProbe(sessionName) {
  ...
  const match = [...client.network].reverse().find(item => {
    ...
    try { const u = new URL(String(item.url || '')); return u.hostname === 'prod-cd-front-ehandel.spendrups.se' && u.pathname === SPENDRUPS_PRODUCT_DETAILS_PATH; } catch { return false; }
  });
  if (!match) throw new Error('no observed Spendrups product detail response; open a product page first');
  const result = await client.send('Network.getResponseBody', { requestId: String(match.requestId || '') }); let raw = String(result?.body || '');
  ...
  let parsed; try { parsed = JSON.parse(raw); } catch { throw new Error('product response was not valid JSON'); }
...
async function categoryProbe(sessionName, pageNumber) {
  ...
  const match = [...client.network].reverse().find(item => {
    if (item?.kind !== 'response' || Number(item.status) !== 200 || !String(item.mimeType || '').includes('application/json')) return false;
    const meta = spendrupsCategoryRequestMeta(item.url); return Boolean(meta && (requestedPage === null || meta.pageNumber === requestedPage));
  });
  ...
  const request = spendrupsCategoryRequestMeta(match.url); if (!request) throw new Error('observed category response failed request validation');
  const result = await client.send('Network.getResponseBody', { requestId: String(match.requestId || '') }); let raw = String(result?.body || '');
  if (result?.base64Encoded) raw = Buffer.from(raw, 'base64').toString('utf8');
  if (Buffer.byteLength(raw, 'utf8') > 768 * 1024) throw new Error('category response exceeded 768 KiB safety limit');
  let parsed; try { parsed = JSON.parse(raw); } catch { throw new Error('category response was not valid JSON'); }
...
async function networkLog(sessionName, limit) { const { client } = await attach(sessionName); const n = Math.max(1, Math.min(200, Number(limit || 100))); return { session: sessionName, entries: client.network.slice(-n) }; }
async function parseBody(req) { const chunks = []; let size = 0; for await (const c of req) { size += c.length; if (size > 32 * 1024) throw new Error('body too large'); chunks.push(c); } if (!chunks.length) return {}; return JSON.parse(Buffer.concat(chunks).toString('utf8')); }
...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.warn(`browser bridge request failed: ${String(error?.message || 'unknown error').replace(/[\r\n]/g, ' ')}`);
    return json(res, 400, { error: 'request rejected' });
  }
}).listen(PORT, '127.0.0.1', () => console.log(`browser bridge listening on ${PORT}; sessions: ${Object.keys(SESSIONS).join(', ')}`));
