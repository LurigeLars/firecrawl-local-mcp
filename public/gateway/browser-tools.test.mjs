...
assert.equal(BROWSER_TOOL_DEFINITIONS.length, 8);
assert.deepEqual([...BROWSER_TOOL_NAMES].sort(), ['browser_category_open','browser_category_probe','browser_network_log','browser_product_open','browser_product_probe','browser_session_open','browser_session_status','browser_snapshot']);
...
assert.deepEqual(browserBridgeRequest('browser_category_open', { session: 'season-spendrups', pageNumber: 5 }), { method: 'POST', path: '/session/category-open', body: { session: 'season-spendrups', pageNumber: 5 } });
assert.throws(() => browserBridgeRequest('browser_category_open', { session: 'season-ms', pageNumber: 5 }), /only for season-spendrups/);
assert.throws(() => browserBridgeRequest('browser_category_open', { session: 'season-spendrups', pageNumber: 0 }), /invalid category page number/);
