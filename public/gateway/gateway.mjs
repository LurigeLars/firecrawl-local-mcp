...
if (!ACCESS_ENABLED && process.env.ALLOW_SECRET_PATH !== '1') {
  console.error('Cloudflare Access (ACCESS_AUD) is not configured; set ALLOW_SECRET_PATH=1 to run on the secret path alone');
  process.exit(1);
}
...
function forward(req, res, body, ctx) {
  ...
  // Never pass client credentials upstream; the upstream uses its own FIRECRAWL_API_URL.
